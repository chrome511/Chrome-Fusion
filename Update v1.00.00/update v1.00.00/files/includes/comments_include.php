...
function showcomments($ctype, $cdb, $ccol, $cid, $clink) {
    ...
    if ((iMODERATOR && checkrights("C")) || (iMEMBER && dbcount("(comment_id)", DB_COMMENTS,
				"comment_id='".$_GET['comment_id']."' AND comment_name='".$userdata['user_id']."'"))) {
			$result = dbquery(
				"DELETE FROM ".DB_COMMENTS."
				WHERE comment_id='".$_GET['comment_id']."'".(iMODERATOR ? "" : "
	...
	if ($settings['comments_enabled'] == "1") {
        ...
        if ((iMODERATOR && checkrights("C")) || (iMEMBER && $data['comment_name'] == $userdata['user_id'] && isset($data['user_name']))) {
