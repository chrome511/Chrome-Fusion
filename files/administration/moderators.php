...
if (isset($_POST['add_mod']) && (isset($_POST['user_id']) && isnum($_POST['user_id']))) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
      ...
      $admin_rights_mod = "B.C.F.M.SU.UL";
		  ...
		  $admin_rights_smod = "A.AC.B.C.F.N.PO.M.MPWR.SU.UL";
...
if (isset($_GET['edit']) && isnum($_GET['edit']) && $_GET['edit'] != 1) {
	...
	if (dbrows($result)) {
		...
		$result2 = dbquery("SELECT admin_rights, admin_title, admin_page FROM ".DB_ADMIN." WHERE admin_page IN (1,2,5) ORDER BY admin_page ASC,admin_title");
      ...
      $risky_rights = array("CP", "APWR", "AD", "MPWR", "MOD");
      ...
      $risky_rights = array("CP", "APWR", "AD", "MPWR", "MOD");
		...
		while ($data2 = dbarray($result2)) {
			if ($page != $data2['admin_page']) {
				...
				if ($page == "2") {
        echo "<tr>\n<td colspan='2' class='tbl2'><strong>".$admin_page['4']."</strong></td>\n</tr>\n<tr>\n";
        } else {
	...
	if (!isset($_POST['search_users']) || !isset($_POST['search_criteria'])) {
		...
		if (isset($result) && dbrows($result)) {
			...
			if ($i > 0) {
				...
				echo $users."<tr>\n<td align='center' class='tbl'>\n";
				echo "<label><input type='checkbox' name='make_mod' value='1' /> ".$locale['415']."</label><br />\n";
	...
	while ($data = dbarray($result)) {
		...
		if ($data['user_level'] >= "102" && $userdata['user_id'] != $data['user_id'] && $userdata['user_level'] != $data['user_level']) {
