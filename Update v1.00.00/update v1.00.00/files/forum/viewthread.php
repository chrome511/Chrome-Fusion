...
if ($rows != 0) {
    ...
    if ($data['user_level'] >= 102) {
        ...
        echo $is_mod ? show_forum_rank($data['user_posts'], 106, $data['user_groups']) : show_forum_rank($data['user_posts'], $data['user_level'], $data['user_groups']);
			} else {
    ...
    if (isset($data['user_web']) && $data['user_web'] && (iMODERATOR || $data['user_status']!=6 && $data['user_status']!=5)) {
			echo "<a href='".$data['user_web']."' target='_blank'><img src='".get_image("web")."' alt='".$data['user_web']."' style='border:0;vertical-align:middle' /></a>"; 
		}
    if (iMEMBER && $data['user_id']!=$userdata['user_id'] && (iMODERATOR || $data['user_status']!=6 && $data['user_status']!=5)) {
