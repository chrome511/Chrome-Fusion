...
+--------------------------------------------------------+
| Filename: moderator_reset.php
| Author: Patrick Kruse (Chrome)
| Based on admin_reset.php
...
if (!checkrights("MPWR") || !defined("iAUTH") || !isset($_GET['aid']) || $_GET['aid'] != iAUTH) { redirect("../index.php"); }

require_once THEMES."templates/admin_header.php";
require_once THEMES."templates/moderator_header.php";
require_once CLASSES."PasswordAuth.class.php";
include LOCALE.LOCALESET."admin/moderator_reset.php";
...
if (isset($_POST['reset_admins']) && isset($_POST['reset_message']) && isset($_POST['reset_admin'])) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		...
		} elseif ($reset_admin == "sm") {
			$user_sql = "user_level='103'";
		} elseif ($reset_admin == "m") {
...
$reset_opts .= "<option value='m'>".$locale['402']."</option>\n";
$reset_opts .= "<option value='sm'>".$locale['403']."</option>\n";
$result = dbquery("SELECT user_id, user_name, user_level FROM ".DB_USERS." WHERE user_level='102' ORDER BY user_level DESC, user_name");
while ($data = dbarray($result)) {
	$reset_opts .= "<option value='".$data['user_id']."'>".$data['user_name']." ".($data['user_level'] == 102 ? "(M)" : "SM")."</option>\n";
}

opentable($locale['mpw_title']);
...
if (!check_admin_pass($admin_password)) {
  ...
  echo "<td class='tbl1'>".$locale['413']."</td>\n";
  } else {
...
$titles = array("all" => $locale['401'], "m" => $locale['402'], "sm" => $locale['403']);
