...
+--------------------------------------------------------+
| Filename: moderators.php
| Author: Patrick Kruse (Chrome)
| Based on administrators.php
...
if (!checkrights("MOD") || !defined("iAUTH") || !isset($_GET['aid']) || $_GET['aid'] != iAUTH) { redirect("../index.php"); }

require_once THEMES."templates/admin_header.php";
require_once THEMES."templates/moderator_header.php";
include LOCALE.LOCALESET."admin/moderators.php";
...
if (isset($_GET['status']) && !isset($message)) {
	if ($_GET['status'] == "sn") {
		...
		$message = $locale['402'];
	} elseif (($userdata['user_level'] == '102') && ($userdata['user_level'] == '103') && $_GET['status'] == "pw") {
		$message = $locale['global_182a'];
...
if (isset($_POST['add_mod']) && (isset($_POST['user_id']) && isnum($_POST['user_id']))) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['make_mod'])) {
      $admin_rights_mod = "M.B.SU.C";
				$admin_rights_mod .= (isset($admin_rights_mod) ? "." : "").
			$result = dbquery("UPDATE ".DB_USERS." SET user_level='".(isset($_POST['make_mod']) ? "102" : "")."', user_rights='$admin_rights_mod' WHERE user_id='".$_POST['user_id']."'");
		} elseif (isset($_POST['make_smod'])) {
		  $admin_rights_smod = "M.MOD.MPWR.B.SU.C";
				$admin_rights_smod .= (isset($admin_rights_smod) ? "." : "").
			$result = dbquery("UPDATE ".DB_USERS." SET user_level='".(isset($_POST['make_smod']) ? "103" : "102")."', user_rights='$admin_rights_smod' WHERE user_id='".$_POST['user_id']."'");
		} else {
...
if (isset($_GET['remove']) && (isset($_GET['remove']) && isnum($_GET['remove']) && $_GET['remove'] != 1)) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		$result = dbquery("UPDATE ".DB_USERS." SET user_admin_password='', user_level='101', user_rights='' WHERE user_id='".$_GET['remove']."' AND user_level='102' OR user_level='103'");
		set_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "");
		redirect(FUSION_SELF.$aidlink."&status=del", true);
	} else {
		if (isset($_POST['confirm'])) {
			echo "<div id='close-message'><div class='admin-message'>".$locale['global_182a']."</div></div>\n";
		}
...
if (isset($_POST['update_admin']) && (isset($_GET['user_id']) && isnum($_GET['user_id']) && $_GET['user_id'] != 1)) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['rights'])) {
			...
			$result = dbquery("UPDATE ".DB_USERS." SET user_rights='$user_rights' WHERE user_id='".$_GET['user_id']."' AND user_level='102' OR user_level='103'");
		} else {
			$result = dbquery("UPDATE ".DB_USERS." SET user_rights='' WHERE user_id='".$_GET['user_id']."' AND user_level='102' OR user_level='103'");
		}
		set_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "");
...
if (isset($_GET['edit']) && isnum($_GET['edit']) && $_GET['edit'] != 1) {
	$result = dbquery("SELECT user_name, user_rights FROM ".DB_USERS." WHERE user_id='".$_GET['edit']."' AND user_level='102' OR user_level='103' ORDER BY user_id");
	if (dbrows($result)) {
		$data = dbarray($result);
		$user_rights = explode(".", $data['user_rights']);
		$result2 = dbquery("SELECT admin_rights, admin_title, admin_page FROM ".DB_ADMIN." WHERE admin_page <=2 ORDER BY admin_page ASC,admin_title");
      ...
      $risky_rights = array("CP", "APWR", "AD", "MPWR", "MOD", "SB", "BB", "DB", "ERRO", "I", "IP", "SL", "P", "PI", "ROB", "U", "S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10", "S11", "S12");
    } else if (dbrows($result)) {
      $result = dbquery("SELECT user_name, user_rights FROM ".DB_USERS." WHERE user_id='".$_GET['edit']."' AND user_level='103' ORDER BY user_id");
      $risky_rights = array("CP", "APWR", "AD", "SB", "BB", "DB", "ERRO", "I", "IP", "SL", "P", "PI", "ROB", "U", "S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10", "S11", "S12");
    }
      ...
      echo $locale['447a']." <input type='password' name='admin_password' class='textbox' style='width:150px;' autocomplete='off' /><br /><br />\n";
      } else {
	...
	if (!isset($_POST['search_users']) || !isset($_POST['search_criteria'])) {
		...
		if (isset($result) && dbrows($result)) {
			...
			if ($i > 0) {
				...
				echo $users."<tr>\n<td align='center' class='tbl'>\n";
				echo "<label><input type='checkbox' name='make_mod' value='1' /> ".$locale['415']."</label><span style='color:red;font-weight:bold;margin-left:5px;'>*</span><br />\n";
				if ($userdata['user_level'] >= 104) { echo "<label><input type='checkbox' name='make_smod' value='1' /> ".$locale['416']."</label><span style='color:red;font-weight:bold;margin-left:5px;'>*</span><br />\n"; }
				if (!check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
          if ($userdata['user_level'] >= '103') {
          echo $locale['447a']." <input type='password' name='admin_password' class='textbox' style='width:150px;' autocomplete='off' /><br /><br />\n";
          } else {
          echo $locale['447']." <input type='password' name='admin_password' class='textbox' style='width:150px;' autocomplete='off' /><br /><br />\n";
				...
				echo "<br />\n<input type='submit' name='add_mod' value='".$locale['417']."' class='button' onclick=\"return confirm('".$locale['461']."');\" />\n";
				echo "</td>\n</tr>\n";
	...
	$i = 0;
	$result = dbquery("SELECT user_id, user_name, user_rights, user_level FROM ".DB_USERS." WHERE user_level='102' OR user_level='103' ORDER BY user_level DESC, user_name");
	echo "<table cellpadding='0' cellspacing='1' width='450' class='tbl-border center'>\n<tr>\n";
