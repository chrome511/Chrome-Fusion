...
if (iMEMBER && (isset($_GET['s_action']) && $_GET['s_action'] == "delete") && (isset($_GET['shout_id']) && isnum($_GET['shout_id']))) {
  if ((iMODERATOR && checkrights("S")) || (iMEMBER && dbcount("(shout_id)", DB_SHOUTBOX, "shout_id='".$_GET['shout_id']."' AND shout_name='".$userdata['user_id']."'"))) {
    $result = dbquery("DELETE FROM ".DB_SHOUTBOX." WHERE shout_id='".$_GET['shout_id']."'".(iMODERATOR ? "" : " AND shout_name='".$userdata['user_id']."'"));
	}
	redirect($link);
}
...
if (iMEMBER || $shout_settings['guest_shouts'] == "1") {
	...
	if (isset($_POST['post_shout'])) {
      ...
      if ((iMODERATOR && checkrights("S")) || (iMEMBER && dbcount("(shout_id)", DB_SHOUTBOX, "shout_id='".$_GET['shout_id']."' AND shout_name='".$userdata['user_id']."'"))) {
				if ($shout_message) {
          $result = dbquery("UPDATE ".DB_SHOUTBOX." SET shout_message='$shout_message' WHERE shout_id='".$_GET['shout_id']."'".(iMODERATOR ? "" : " AND shout_name='".$userdata['user_id']."'"));
				}
			}
			redirect($link);
