...
if (isset($_POST['reset_admins']) && isset($_POST['reset_message']) && isset($_POST['reset_admin'])) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		...
		if (isnum($reset_admin)) {
			...
			$user_sql = "user_level='104' OR user_level='105'";
		} elseif ($reset_admin == "sa") {
			$user_sql = "user_level='105'";
		} elseif ($reset_admin == "a") {
			$user_sql = "user_level='104'";
		} else {
...
$result = dbquery("SELECT user_id, user_name, user_level FROM ".DB_USERS." WHERE user_level>='103' ORDER BY user_level DESC, user_name");
while ($data = dbarray($result)) {
	$reset_opts .= "<option value='".$data['user_id']."'>".$data['user_name']." ".($data['user_level'] == '104' ? "(A)" : "(SA)")."</option>\n";
}
