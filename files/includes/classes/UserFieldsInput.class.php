...
class UserFieldsInput {
	...
	private function _setNewAdminPassword() {
		...
		if (iMODERATOR && $this->_method == "validate_update" && !$this->isAdminPanel && $this->userData['user_level'] == "102" || $this->userData['user_level'] == "103") {
				...
				if ($this->_isValidCurrentPassword) {
					if ($this->_userAdminPassword != $this->_userPassword) {
						...
						if ($_isValidNewPassword === 0) {
							...
							if ($_isValidNewPassword === 1) {
								// New Password equal old password
								$this->_setError("user_password", $locale['u144a'].$locale['u146'].$locale['u131a']);
							} elseif ($_isValidNewPassword === 2) {
								// The two new passwords are not identical
								$this->_setError("user_password", $locale['u148b']);
							} elseif ($_isValidNewPassword === 3) {
								// New password contains invalid chars / symbols
								$this->_setError("user_password", $locale['u144a'].$locale['u142']."<br />".$locale['u147']);
							}
						}
					} else {
					...
					$this->_setError("user_admin_password", $locale['u149d']);
				}
			} else {
				// Current admin password is invalid
				$this->_setError("user_admin_password", $locale['u149c'], $showError);
			}
		} else if (iADMIN && $this->_method == "validate_update" && !$this->isAdminPanel && $this->userData['user_level'] >= "104") {
