...
// Deactivate Inactive Users
  ...
  $inactive = dbcount("(user_id)", DB_USERS, "user_status='0' AND user_level<'105' AND user_lastvisit<'$time_overdue' AND user_actiontime='0'");
    ...
    $result = dbquery(
			...
			WHERE user_level<'105' AND user_lastvisit<'".$time_overdue."' AND user_actiontime='0' AND user_status='0'
			LIMIT 0,50"
		);
...
// Delete User
    ...
    $result = dbquery("SELECT user_id, user_avatar FROM ".DB_USERS." WHERE user_id='".$user_id."' AND user_level<'105'");
		if (dbrows($result)) {
			// Delete avatar
			$data = dbarray($result);
...
// Ban User
  ...
  $result = dbquery("SELECT user_name, user_email, user_status FROM ".DB_USERS." WHERE user_id='".$user_id."' AND user_level<'105'");
	if (dbrows($result)) {
...
$result = dbquery("SELECT user_status FROM ".DB_USERS." WHERE user_id='".$user_id."' AND user_level<'105'");
	if (dbrows($result)) {
		$udata = dbarray($result);
...
// Deactivate User
  ...
  $rows = dbcount("(user_id)", DB_USERS, "$user_name user_status='$usr_mysql_status' AND user_level<'105'");
  $result = dbquery(
		"SELECT user_id, user_name, user_level FROM ".DB_USERS."
		WHERE $user_name user_status='$usr_mysql_status' AND user_level<'105'
  ...
  if ($settings['enable_deactivation'] == 1) {
    if (dbcount("(user_id)", DB_USERS, "user_status='0' AND user_level<'105' AND user_lastvisit<'$time_overdue' AND user_actiontime='0'")) {
