...
class UserFields {
	...
	public function displayOutput() {
		...
		$this->renderFields();
		if (iMODERATOR && checkrights("M")) { $this->renderIPOutput(); }
		if ($this->userData['user_groups']) { $this->renderUserGroups(); }
		if ($this->showAdminOptions && iMODERATOR && checkrights("M") && $this->userData['user_id'] != $userdata['user_id'] && $this->userData['user_level'] < 102) {
	...
	private function renderBasicInputFields() {
		...
		// Admin Password
		if ($this->showAdminPass && ($this->userData['user_level'] == "102" || $this->userData['user_level'] == "103")) {
			$this->html .= "<tr>\n<td colspan='2' class='profile_category_name tbl2'><strong>".$locale['u130a']."</strong></td></tr>\n";
			if ($this->userData['user_admin_password']) {
				$this->html .= $this->basicInputField("user_admin_password", $locale['u131a'], "64", "", "password", false, "user_admin_password");
			}
			$this->html .= $this->basicInputField("user_new_admin_password", ($this -> userData['user_admin_password'] ? $locale['u144a'] : $locale['u131a']), "64", "", "password", false, "user_admin_password");
			$this->html .= "<tr>\n<td class='tbl'></td>\n<td class='tbl'><span class='small2'>".$locale['u147']."</span></td>\n</tr>\n";
			$this->html .= $this->basicInputField("user_new_admin_password2", $locale['u145a'], "64", "", "password", false, "user_admin_password");
		} else if ($this->showAdminPass && ($this->userData['user_level'] >= "104")) {
		...
		$this->html .= (iMODERATOR || $this->_userNameChange ? $this->basicInputField("user_name", $locale['u127'], "30", $locale['u122']) : "");
	...
	private function renderBasicOutputFields() {
		...
		if (iMODERATOR || $this->userData['user_hide_email'] == 0) {
			$rowspan = $rowspan+1;
		...
		if (iMEMBER && $userdata['user_id'] != $this->userData['user_id']) {
			...
			if (iMODERATOR && checkrights("M") && $this->userData['user_level'] != "105" && $this->userData['user_id'] != "1") {
		...
		if (iMODERATOR && $this->userData['user_status'] > 0) {
	...
	private function renderAdminOptions() {
		...
		if (count($groups_cache) > 0) {
			...
			if (iMODERATOR && checkrights("UG") && $user_groups_opts) {
