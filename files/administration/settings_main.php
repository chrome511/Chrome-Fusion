...
if (isset($_POST['savesettings'])) {
	...
	if (!$result) { $error = 1; }
	$result = dbquery("UPDATE ".DB_SETTINGS." SET settings_value='".stripinput($_POST['siteadminemail'])."' WHERE settings_name='siteadminemail'");
	if (!$result) { $error = 1; }
	...
	if (($localeset != $old_localeset) && !$error) {
		...
		if (!$result) { $error = 1; }
		$result = dbquery("UPDATE ".DB_ADMIN." SET admin_title='".$locale['201a']."' WHERE admin_link='moderators.php'");
		if (!$result) { $error = 1; }
		$result = dbquery("UPDATE ".DB_ADMIN." SET admin_title='".$locale['248a']."' WHERE admin_link='moderator_reset.php'");
		if (!$result) { $error = 1; }
...
echo "</tr>\n<tr>\n";
echo "<td width='30%' class='tbl'>".$locale['405a']."</td>\n";
echo "<td width='70%' class='tbl'><input type='text' name='siteadminemail' value='".$settings2['siteadminemail']."' maxlength='128' class='textbox' style='width:230px;' /></td>\n";
echo "</tr>\n<tr>\n";
